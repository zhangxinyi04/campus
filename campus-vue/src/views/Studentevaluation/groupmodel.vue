<template>
  <div v-loading='loading'>
    ...
    <div class="student-review-container" v-else>
      ...
      <!-- <div v-for="(student,index) in studentsgroup" :key="student.studentId" class="student-card"
      ...
      <div class="group-container">
        <!-- 分组学生 -->
        <template v-for="(group, groupId) in studentsgroup">
          <div v-if="groupId != 'null'" :key="groupId" class="group-section">
            <h2 class="group-title">{{ groupId }} 组</h2> <el-button type="primary" @click="Groupreview(group)"
              icon="el-icon-star-off" style="float: right;margin-top: -40px;">
              小组点评
            </el-button>
            <transition-group name="list" tag="div" class="student-list">
              <div v-for="student in group" :key="student.studentId" class="student-card"
                :class="{ 'selected': selectedIds.includes(student.studentId) }"
                @click="handleSelect(student.studentId,student)">
                <!-- 原有学生卡片内容保持不变 -->
                <div v-if="isMultiSelect" class="checkbox-wrapper">
                ...
                <div class="item-badge">{{ student.counts }}</div>
                <img :src="student.sex ==='0' ? man : woman" alt="" style="width: 50px;height: 50px;">
                <h3 class="name" style="font-size: 14px;">{{ student.studentName }}</h3>
              </div>
            </transition-group>
          </div>
        </template>
        ...
        <div class="group-section">
          ...
          <transition-group name="list" tag="div" class="student-list">
            <div v-for="student in studentsgroup[null]" :key="student.studentId" class="student-card"
              :class="{ 'selected': selectedIds.includes(student.studentId) }"
              @click="handleSelect(student.studentId,student)">
              <!-- 原有学生卡片内容保持不变 -->
              <div v-if="isMultiSelect" class="checkbox-wrapper">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      sortedGroups() {
        const groups = Object.entries(this.studentsgroup)
          .filter(([key]) => key !== null)
          .sort((a, b) => a[0] - b[0]);

        return [
          ...groups,
          ['null', this.studentsgroup[null]]
        ];
      }
    },
    created() {
    ...
    methods: {
      Groupreview(group) {
        if (this.$store.state.Floating.count) {
            const studentName = group.map(item => item.studentName).join('、');
            const studentArr = Object.values(group).flat().map(item => ({
              studentId: item.studentId,
              deptId:this.id.deptId,
              courseId :this.id.courseId,
              bonusTypeId : this.classEvaluationlistArr[0] ? this.classEvaluationlistArr[0].bonusTypeId : '',
              type : this.classEvaluationlistArr[0] ? this.classEvaluationlistArr[0].type : '',
              mark : this.classEvaluationlistArr[0] ? this.classEvaluationlistArr[0].mark : ''
            }));
             classEvaluation(studentArr).then(response => {
               this.$message({
                 message: studentArr[0].type === '0' ?
                   `${studentName} +${studentArr[0].mark}` : `${studentName} -${studentArr[0].mark}`,
                 type: studentArr[0].type === '0' ? 'success' : 'warning'
               });
          ...
          this.selectedIds = group.map(student => student.studentId)
          // this.SelectSelect()
          this.obj = []
          var objs = this.id
      ...
      selectStudents(deptId) {
        ...
        selectStudents({
          ...
          this.loading = false
          // console.log(this.studentsgroup,'this.students')
        });
      },
      // selectClass() {
      ...
      classEvaluationlist0() {
        classEvaluationlist({
          ...
          // console.log(this.classEvaluationlistArr)
        });
      },
