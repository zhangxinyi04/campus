...
//    @PreAuthorize("@ss.hasPermi('system:user:query')")
    ...
    public AjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId) {
        ...
        if (StringUtils.isNotNull(userId)) {
            ...
            ajax.put("courseIds", sysCourseService.selectCourseByUserId(userId));
            ajax.put("roleIds", sysUser.getRoles().stream().map(SysRole::getRoleId).collect(Collectors.toList()));
        }
        List<SysRole> roles = roleService.selectRoleAll();
        ajax.put("roles", SysUser.isAdmin(userId) ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
        ajax.put("posts", postService.selectPostAll());
        ajax.put("course", sysCourseService.selectSysCourseList(null));
        return ajax;
    }

    @GetMapping(value = {"/checkCourse"})
    public AjaxResult checkCourse() {
        return AjaxResult.success(sysCourseService.selectCourseByUserId(null));
    }
     ...
     * 查询家长信息列表
     */
    @PreAuthorize("@ss.hasPermi('system:user:listPatriarch')")
    @GetMapping("/listPatriarch")
    public AjaxResult listPatriarch(SysUser user) {
        List<SysUser> list = userService.selectSysPatriarchList(user);
        return success(list);
    }
}
