			...
			<view class="entry-container">
				<view class="entry-item" @tap="navTo('parent',term)">
					<view class="entry-left">
						...
						<text class="entry-text">教师寄语</text>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			_self = this
			_self.studentId = options.studentId
			// console.log(options.studentId,'optionsoptions')
			_self.commentlist()
		},
		methods: {
			commentlist() {
			...
			navTo(type, term) {
				...
				if (type === 'parent') {
					url =
						`/pages/PersonalCenter/teacherEvaluation/Parentsmessage?studentCommentId=${term.studentCommentId}&studentId=${_self.studentId}`;
				} else {
