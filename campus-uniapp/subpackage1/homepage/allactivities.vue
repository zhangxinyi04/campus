<template>
	<view class="container">
		<!-- Tab切换 -->
		<view class="tabs">
			<view class="tab-item" :class="{ active: activeTab === 0 }" @click="switchTab(0)">校级活动</view>
			<view class="tab-item" :class="{ active: activeTab === 1 }" @click="switchTab(1)">班级活动</view>
			<view class="tab-line" :style="lineStyle"></view>
		</view>

		<!-- 搜索框 -->
		<view class="search-box">
			<input class="search-input" placeholder="搜索活动名称" placeholder-class="placeholder" />
			<text class="iconfont icon-search"></text>
		</view>

		<!-- 活动列表 -->
		<scroll-view scroll-y class="activity-list">
			<view class="activity-item" v-for="(item,index) in eventslistdata" :key="index"
				@click="EventDetailsclick(item)">
				<image class="cover" :src="configUrl+item.eventPic" mode="aspectFill" />
				<view class="content">
					<!-- 头部信息 -->
					<view class="header">
			...
			<view class="status-tag" class="ended" v-if='item.status == 2'>
			    已结束
					...
					<view class="tag-group">
						...
						<view class="custom-tag" v-for="(tag,tIndex) in item.tags" :key="tIndex">
							{{ tag }}
						</view>
					</view>
				</view>
		...
		methods: {
			...
			studentgetInfo() {
				this.$request.post(studentgetInfo, {}).then((res) => {
					_self.gradeIds.push(res.data.student.dept.parentId);
					_self.classIds.push(res.data.student.dept.deptId);
					_self.eventslist()
				}, (err) => {
			...
			EventDetailsclick(item) {
				let url = `/subpackage1/homepage/EventDetails?eventId=${item.eventId}`;
				uni.navigateTo({
					url: url
				});
			},
